...
public class WxUserService {
	...
	public void syncWxUser(){
		...
		wxUserDao.truncateWxUser();
		JsonArray ja = jb.get("data").getAsJsonObject().get("openid").getAsJsonArray();
		for (int i = 0; i < ja.size(); i++) {
			String openid = ja.get(i).getAsString();
			url = String.format( WxConsts.USER_QUERY_INFO_URL, access_token,openid,"zh_CN");
			WxUserInfo wxUserInfo = GsonUtils.fromJson(HttpClientUtils.get(url), WxUserInfo.class);
